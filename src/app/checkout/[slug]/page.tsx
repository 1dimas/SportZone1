...
export default function CheckoutPage({ params }: CheckoutPageProps) {
  ...
  const appFee = 1000;
  const total =
    (product?.price || 0) + shippingCost + insuranceCost + serviceFee + appFee;
              ...
              <div className="flex flex-col gap-2 mt-2">
                ...
                <div className="border rounded p-3 flex flex-col gap-2">
                  ...
                  <div className="text-xs text-gray-500">
                    J&T Kargo (20.000){" "}
                    <span className="ml-2">Estimasi tiba besok - 15 Sep</span>
                  </div>
                  <label className="flex items-center gap-2 text-sm">
                    <input
                      type="checkbox"
                    ...
                    />
                    Pakai Asuransi Pengirim (
                    Rp{insuranceCost.toLocaleString("id-ID")})
                  </label>
                </div>
                <div className="flex flex-col mt-2">
                  <label htmlFor="note" className="text-xs text-gray-600 mb-1">
                    Kasih Catatan
                  </label>
                  <textarea
                    id="note"
          ...
          <div className="bg-white rounded-xl p-6 shadow flex flex-col gap-6">
            ...
            <div className="flex flex-col gap-3">
              {paymentMethods.map((method) => (
                <label
                  key={method.name}
                  className="flex items-center gap-3 cursor-pointer py-2 px-2 rounded hover:bg-orange-50 transition"
                >
                  <input
                    type="radio"
                    name="payment"
            ...
            <div className="border-t border-gray-200 pt-4">
              <div className="font-semibold mb-2">Cek Ringkasan Transaksimu</div>
            ...
            <button className="w-full bg-orange-500 hover:bg-orange-600 text-white font-bold py-3 rounded-lg transition-all duration-300 shadow hover:shadow-md text-lg">
              Bayar Sekarang
            </button>
          </div>
        </div>
