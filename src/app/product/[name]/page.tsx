...
import type { Product, ProductVariant } from "@/app/data/products"; // Import tipe baru
...
export default function ProductPage({ params }: ProductPageProps) {
  ...
  const productSlug = decodeURIComponent(params.name).toLowerCase();

  // --- Cari produk berdasarkan slug (case insensitive) ---
  ...
  if (!product) {
    return (
      <div className="flex h-screen items-center justify-center text-center text-red-600">
        <h1>Produk dengan slug {params.name} tidak ditemukan.</h1>
      </div>
    );
  }
