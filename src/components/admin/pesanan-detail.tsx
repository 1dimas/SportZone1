  ...
  const fetchPesanan = async () => {
    try {
      ...
      if (data.pesanan_items && data.pesanan_items.length > 0) {
        ...
        await Promise.all(
          data.pesanan_items.map(async (item) => {
            if (item.id_produk && !item.produk?.gambar?.length) {
              try {
                const produk = await getProdukById(item.id_produk);
                if (produk) {
                  const productImages = produk.gambar || [];
                  images[item.id_produk] = Array.isArray(productImages) && productImages.length > 0
                    ? productImages[0]
                    : produk.gambar_url || "/products/kao.jpeg";
                }
              } catch (err) {
                console.error(`Failed to fetch product ${item.id_produk}:`, err);
                images[item.id_produk] = "/products/kao.jpeg";
              }
            } else if (item.produk?.gambar?.length) {
              images[item.id_produk] = item.produk.gambar[0];
            } else if (item.produk?.gambar_url) {
              images[item.id_produk] = item.produk.gambar_url;
            } else {
              images[item.id_produk] = "/products/kao.jpeg";
            }
          })
          ...
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
            ...
            <div className="divide-y divide-gray-50">
              {pesanan.pesanan_items?.map((item) => {
                ...
                const imageUrl = productImages[item.id_produk] || "/products/kao.jpeg";

                return (
                  <div key={item.id} className="px-5 py-4 flex items-center gap-4">
                    <div className="w-14 h-14 bg-gray-50 rounded-lg flex-shrink-0 border border-gray-200 overflow-hidden">
                      <img
                        src={imageUrl}
                        alt={productName}
                        className="w-full h-full object-cover"
                        onError={(e) => {
                          (e.target as HTMLImageElement).src = "/products/kao.jpeg";
                        }}
                      />
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      {item.produk_varian && (
                        <p className="text-xs text-gray-400 mt-0.5">
                          {item.produk_varian.warna_varian && `${item.produk_varian.warna_varian} • `}
                    ...
                    <div className="text-right">
                      <p className="text-sm text-gray-500">
                        {item.kuantitas} × Rp {Number(item.harga_satuan).toLocaleString("id-ID")}
                      </p>
                      <p className="font-medium text-gray-900">
                        Rp {(Number(item.harga_satuan) * item.kuantitas).toLocaleString("id-ID")}
                      </p>
                    </div>
                  </div>
                );
